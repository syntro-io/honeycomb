...
describe "Reporting value data into IR using Honeycomb.perf_data" do
  it "adds key/value pairs into the IR output" do
    ir = Honeycomb::IR.load("examples/cucumber/out.res")
    expect(ir.tests.first).to have_key(:values)
    expect(ir.tests.first[:values]).to be_a Hash
  end
end

describe "Honeycomb::IR#values" do
...
eos
    ...
    it "identifies values at the top level" do
      ir = Honeycomb::IR.load(TMP_FILE + '-top')
...
eos
    ...
    it "identifies values at the top level" do
      ir = Honeycomb::IR.load(TMP_FILE + '-mixed')
