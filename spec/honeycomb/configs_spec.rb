...
describe Honeycomb::Config do

  describe ".new" do
    it 'can be initialized with required parameters' do
      expect( Honeycomb::Config.new([:a, :b]) ).to be_a Honeycomb::Config
    end
  end
  ...
  describe "#process" do
    ...
    context "Initialized with required values" do
      ...
      let!(:config) { Honeycomb::Config.new([:a, :b], :pre_env => 'PRE_ENV_') }
    ...
    context "Initialized with required and optional values" do
      let!(:config) { Honeycomb::Config.new([:a, :b], :optional => [:c, :d], :pre_env => 'PRE_ENV_') }
