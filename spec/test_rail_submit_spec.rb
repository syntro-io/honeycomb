...
describe Honeycomb::Reporters::TestRail do

  describe "Submitting Junit results in testrail" do
    describe "Create a new test run and submit results" do

      junit_command = "honeycomb --junit './examples/junit/junittests.xml' --submit testrail --config-file ./examples/cucumber/.test_rail.yaml"
  ...
  describe "Submitting Cucumber results in testrail" do
    describe "Create a new test run and submit results" do
      ...
      it "Submit the test results", :if => File.exists?("./examples/cucumber/.test_rail.yaml") do
        `#{cucumber_command} features/tags.feature -f Honeycomb::Formatters::RubyCucumber -o ./cucumber.res`
        @output = `unset BUNDLE_GEMFILE; unset RUBYOPT; res --honeycomb examples/cucumber/cucumber.res --submit testrail --config-file ./examples/cucumber/.test_rail.yaml`
  ...
  describe "Submitting Rspec results in testrail" do
    describe "Create a new test run and submit results" do
      ...
      it "Submit the test results", :if => File.exists?("./examples/cucumber/.test_rail.yaml") do
        `#{rspec_command} -f Honeycomb::Formatters::Rspec -o ./rspec.res`
        @output = `res --honeycomb ./rspec.res --submit testrail --config-file ./examples/cucumber/.test_rail.yaml`
        expect( @output ).to include ("Submitted to Test Rail")
      end
